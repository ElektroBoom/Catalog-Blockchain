...
class Carnet:
    ...
    def sign(self, emitator, receptor, info_didactic):
    ...
    def verify_rezultat(rezultat):
        public_key = RSA.importKey(binascii.unhexlify(rezultat.emitator))
        verifier = PKCS1_v1_5.new(public_key)
        h = SHA256.new((str(rezultat.emitator) + str(rezultat.receptor) +
                        str(rezultat.info_didactic)).encode('utf8'))
        return verifier.verify(h, binascii.unhexlify(rezultat.signature))
